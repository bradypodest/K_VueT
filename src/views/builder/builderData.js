...
let data = {
    dialogAddOptions: [
        [{ "title": "父级ID", "field": "RefID", "required": true, type: 'string' }],
        [{
            "title": "项目命名空间", "field": "namespace", "placeholder": "项目命令空间(类库名)", "type": "string", "required": true, 
        }],
    ...
    form: {
        data: {
            ID: '', parentId: 0, namespace: '',
            cName: '', tableName: '', folderName: '',
            detailCnName: '', detailName: '',sortName: '',
            vuePath: '',
        },
        options: [
            [
                { "title": "ID", "field": "ID",  readonly: true, disabled: true, type: 'string' },
                { "title": "父级ID", "field": "RefID",  "required": true, type: 'string' },
                {
                    "title": "项目命名空间", "placeholder": "项目命令空间(类库名)", "field": "namespace", "type": "string", "required": true
                }
            ],
            [
                { "title": "表中文名", "field": "cNName", "required": true },
                { "title": "表名", placeholder: "默认与实际表名相同", "field": "tableName", "required": true },
                { "title": "项目文件夹", placeholder: "生成文件所在类库中的文件夹名(文件夹可以不存在)", "field": "folderName", "required": true },
            ],
            [
                { "title": "明细表名", placeholder: "多个明细表用逗号隔开", "field": "detailName" },
                { "title": "明细表中文名",  placeholder: "多个明细表用逗号隔开",  "field": "detailCnName" },
                { "title": "排序字段",  placeholder: "现阶段会默认为createTime",  "field": "sortName", "placeholder": "非自增主键需要输入排序字段" },
            ]
            // [ //待完
