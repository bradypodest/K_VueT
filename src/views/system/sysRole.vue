    ...
    <div class="filters">
      <el-form :model="filtersData" @submit.native.prevent>
        <el-row>
          ...
          <el-col :span="6" :offset="tableOptions.searchForm&&tableOptions.searchForm.length>0?1:0">
            <el-form-item>
        ...
        >
          <template slot-scope="scope">
            <a
              ...
              @click="link(scope.row,col)"
              v-if="col.type=='link'"
              v-text="scope.row[col.prop]"
            ></a>
            <img
              :onerror="defaultImg"
              @click="viewImg(scope.row,col)"
              class="table-img"
              v-else-if="col.type=='img'"
              :src="scope.row[col.prop]"
            />
            <div v-else-if="col.type=='date'">{{formatterDate(scope.row,col)}}</div>
  ...
  methods: {
      ...
      that.dialogFormOptions = [
        {
          ...
          in_size: "small", //控件大小
          inp_holder: "请输入角色名称" //提示信息
        },
        {
          type: "textarea", //控件类型：text为input控件，select为下拉框,
          data: null, //如果为下拉框，下拉框的数据
          prop: "Description", //标记
          label: "描述", //label值
          lb_width: "100px", //label长度
          in_size: "small", //控件大小
          inp_holder: "请输入角色描述" //提示信息
        }
      ];
    },
    //获取设置 弹出层编辑规则
    ...
    */
    ...
    formatterDate(row, column) {
      return (row[column.prop] || "").substr(0, 10);
    }
    //其它  end
  },
  //页面挂载之后执行方法
